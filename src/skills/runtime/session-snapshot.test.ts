...
describe("resolveReusableWorkspaceSkillSnapshot", () => {
  ...
  it("keeps ordinary child snapshots reusable without workspace-only policy", () => {
    shouldRefreshSnapshotForVersionMock.mockReturnValue(false);
    const existingSnapshot = strippedSnapshot();
    ...
    expect(result.snapshot.workspaceOnly).toBeUndefined();
  });
